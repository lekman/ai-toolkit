...
/**
 * The plugin marketplace, cloned into the image at build time.
 *
 * Baked rather than fetched at start: a containerised task is often
 * unattended, and a start that depends on a network round trip is a start that
 * can hang. Root-owned, so the agent cannot rewrite the skills it is running
 * under.
 *
 * It is a snapshot, and `--rebuild` is how it moves forward. The commit is not
 * part of the image tag, so a marketplace change does not rebuild on its own —
 * see `marketplaceRef` for why the rebuild has to be told which commit it is
 * fetching rather than being left to say "main".
 */
export const MARKETPLACE_REPO = "https://github.com/lekman/ai-toolkit.git";
export const MARKETPLACE_NAME = "ai-toolkit";
export const MARKETPLACE_DIR = "/opt/ai-toolkit";
/** Where a mounted Obsidian vault appears inside the container. */
export const CONTAINER_VAULT = `${CONTAINER_HOME}/vault`;
...
# Skills from the marketplace baked into the image at ${MARKETPLACE_DIR}.
#
# Registered from that local directory rather than from GitHub, so this needs
# no network and cannot stall a start. Both commands write into the config
# volume, so they are a first-run cost per volume rather than per session —
# they are still run every time because they are idempotent and a volume
# created by an older version would otherwise never catch up.
if [ -d "${MARKETPLACE_DIR}" ] && [ -n "\${CD_PLUGINS:-}" ]; then
  claude plugin marketplace add "${MARKETPLACE_DIR}" >/dev/null 2>&1 || true
  IFS=',' read -ra plugins <<< "\${CD_PLUGINS}"
  for plugin in "\${plugins[@]}"; do
    [ -z "\${plugin}" ] && continue
    claude plugin install "\${plugin}@${MARKETPLACE_NAME}" >/dev/null 2>&1 \\
      || echo "[claude-docker] could not install \${plugin}@${MARKETPLACE_NAME}" >&2
  done
fi
...
# The plugin marketplace, fetched at one commit. Public repo, so no key is
# needed and nothing of yours is embedded. Root-owned: the agent reads the
# skills it runs under and cannot rewrite them.
#
# The commit is a build argument, and it appears in this RUN line, so the layer
# cache breaks exactly when the marketplace moves. A plain \`git clone\` here
# does not: its command text never changes, so docker reuses the cached layer
# and a rebuild silently ships the marketplace as it was on the day the layer
# was first written — a build that reports success having changed nothing.
ARG MARKETPLACE_REF=main
RUN mkdir -p ${MARKETPLACE_DIR} \\
    && git -C ${MARKETPLACE_DIR} init -q \\
    && git -C ${MARKETPLACE_DIR} remote add origin ${MARKETPLACE_REPO} \\
    && git -C ${MARKETPLACE_DIR} fetch -q --depth 1 origin "\${MARKETPLACE_REF}" \\
    && git -C ${MARKETPLACE_DIR} checkout -q FETCH_HEAD \\
    && git -C ${MARKETPLACE_DIR} rev-parse HEAD > ${MARKETPLACE_DIR}/.baked-ref \\
    && chmod -R a-w ${MARKETPLACE_DIR}

RUN useradd --create-home --uid ${CONTAINER_UID} --shell /bin/bash ${CONTAINER_USER} \\
    && mkdir -p ${WORKDIR} ${CONTAINER_HOME}/.claude ${CONTAINER_VAULT} \\
...
/**
 * The marketplace commit to bake in: the remote's current default branch head.
 *
 * Resolved on the host rather than left to `main` inside the build, because the
 * value has to *change* for docker to rediscover that the clone layer is stale.
 * Only called when a build is already happening, so it adds no round trip to an
 * ordinary launch.
 *
 * Falls back to the branch name when the remote cannot be reached. The build
 * then still works and still pins to whatever `main` is at that moment; it just
 * loses the cache-busting property, which is no worse than not asking.
 */
export function marketplaceRef(): string {
  const result = spawnSync("git", ["ls-remote", MARKETPLACE_REPO, "HEAD"], {
    encoding: "utf8",
    timeout: 20_000,
  });
  const sha = result.stdout?.trim().split(/\s+/)[0];
  return sha && /^[0-9a-f]{40}$/.test(sha) ? sha : "main";
}
...
export async function buildImage(tag: string): Promise<void> {
  ...
  console.error(
    `[claude-docker] marketplace at ${ref === "main" ? "main (remote unreachable, cache may hold)" : ref.slice(0, 12)}`,
  );
  ...
  const result = spawnSync(
    "docker",
    ["build", "-t", tag, "--build-arg", `MARKETPLACE_REF=${ref}`, "-"],
    {
      input: context,
