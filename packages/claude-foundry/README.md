...
| `--capacity <n>` | SKU capacity in thousands of tokens per minute. The catalog default is 10, which throttles a real session — raise it if you have the quota |
| `--hosting azure\|anthropic` | Force a hosting variant instead of taking the catalog default |
...
granted per model. The models appear in the catalog either way, which is why the
...
`modelProviderData` block — organization name, industry, and two-letter country
...
your account — and because Foundry does not check models at startup, that fails
