...
const { Plugin, ItemView, Notice, MarkdownRenderer, PluginSettingTab, Setting, Menu, Modal } = obsidian;
...
const VIEWS = [
  ...
  { key: 'all', label: 'All', columns: ['today', 'tomorrow', 'future', 'unscheduled'] },
];
...
function runScript(plugin, key, flags = []) {
  return new Promise((resolve) => {
    ...
    execFile(resolveBun(plugin.settings), [script, ...flags], { cwd: dir, timeout: 60000 },
      (err, stdout, stderr) => {
...
class ConfirmRunModal extends Modal {
  ...
  onOpen() {
    ...
    for (const line of this.preview) body.createDiv({ cls: 'dk-confirm-line', text: line });
    const row = this.contentEl.createDiv({ cls: 'dk-confirm-actions' });
    const cancel = row.createEl('button', { cls: 'dk-btn', text: 'Cancel' });
    cancel.addEventListener('click', () => this.close());
    const go = row.createEl('button', { cls: 'dk-btn mod-cta', text: this.title });
    go.addEventListener('click', () => { this.close(); this.onConfirm(); });
  }
...
function menuHasSubmenus() {
  if (submenuSupport === null) {
    ...
    try {
      new Menu().addItem((i) => { submenuSupport = typeof i.setSubmenu === 'function'; });
    } catch {
      submenuSupport = false;
    }
  }
...
function parseDashboard(text) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (/^##\s+Focus\s*$/.test(raw)) { inFocus = true; continue; }
    if (!inFocus) continue;
    if (/^##\s+/.test(raw) && !/^##\s+Focus/.test(raw)) break; // ## Initiatives ends Focus

    if (/^>\s*\[!note\]-\s*Tomorrow/.test(raw)) {
      band = 'tomorrow'; day = null; client = null; run = null; continue;
    }
    if (/^>\s*\[!note\]-\s*Future/.test(raw)) {
      band = 'future'; day = null; client = null; run = null; continue;
    }
    ...
    if (band !== 'today') {
      if (/^>/.test(raw)) {
        ...
        band = 'today'; day = null; client = null; run = null; continue;
      } else {
        band = 'today'; line = raw; depth = 0;
      }
    }
    ...
    if (runMatch && !/^\s*[-*]/.test(line)) { run = runMatch[1]; continue; }
...
function itemBody(text) {
  const bold = text.match(/\*\*(.+?)\*\*/);
  if (!bold) return '';
  return text.slice(text.indexOf(bold[0]) + bold[0].length).replace(/^\s*[.·:]\s*/, '').trim();
}
...
async function snapshot(plugin, text) {
  try {
    ...
    const path = require('path');
    const os = require('os');
    const dir = plugin.settings.snapshotDir || path.join(os.homedir(), '.claude', 'dashboard-snapshots');
    fs.mkdirSync(dir, { recursive: true });
    const stamp = new Date().toISOString().replace(/[:.]/g, '-');
    fs.writeFileSync(path.join(dir, `Dashboard-${stamp}-kanban.md`), text, 'utf8');
  } catch (err) {
...
async function editDashboard(plugin, expectLine, expectRaw, mutate) {
  const file = plugin.app.vault.getAbstractFileByPath(plugin.settings.dashboardPath);
  if (!file) { new Notice('Dashboard not found: ' + plugin.settings.dashboardPath); return false; }
  const text = await plugin.app.vault.read(file);
  const lines = text.split('\n');
  if (expectLine !== null && lines[expectLine] !== expectRaw) {
    new Notice('Dashboard changed underneath the board. Refreshed instead of writing.');
    plugin.refreshViews();
    return false;
  }
...
function locateInsertPoint(lines, column, client) {
  ...
  const strip = (l) => (pre ? (l.startsWith('>') ? l.replace(/^>\s?/, '') : null) : l);
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (/^##\s+Focus\s*$/.test(raw)) { inFocus = true; continue; }
    if (!inFocus) continue;
    if (/^##\s+/.test(raw) && !/^##\s+Focus/.test(raw)) { if (dayStart >= 0 && dayEnd < 0) dayEnd = i; break; }

    if (/^>\s*\[!note\]-\s*Tomorrow/.test(raw)) { if (dayStart >= 0 && dayEnd < 0) dayEnd = i; band = 'tomorrow'; continue; }
    if (/^>\s*\[!note\]-\s*Future/.test(raw)) { if (dayStart >= 0 && dayEnd < 0) dayEnd = i; band = 'future'; continue; }

    let line = raw;
    if (band !== 'today') {
      if (/^>/.test(raw)) line = raw.replace(/^>\s?/, '');
      else if (raw.trim() === '') { if (dayStart >= 0 && dayEnd < 0) dayEnd = i; band = 'today'; continue; }
      else { band = 'today'; line = raw; }
    }
    ...
    if (dayMatch) {
      ...
      if (col === column && dayStart < 0) { dayStart = i; curDay = name; }
    }
  }
  ...
  if (client === NO_CLIENT) {
    ...
    for (let i = dayStart + 1; i < dayEnd; i++) {
      ...
      if (line !== null && /^####\s+/.test(line)) { end = i; break; }
    }
    let lastOpen = -1;
    ...
    if (lastOpen < 0) while (insert < end && (strip(lines[insert]) || '').trim() === '') insert++;
  ...
  for (let i = dayStart + 1; i < dayEnd; i++) {
    ...
    if (groupStart >= 0) { groupEnd = i; break; }
  ...
  if (groupStart < 0) {
    ...
    let end = dayEnd;
    while (end > dayStart + 1 && (strip(lines[end - 1]) || '').trim() === '') end--;
...
class KanbanView extends ItemView {
  ...
  async render() {
    ...
    const file = this.app.vault.getAbstractFileByPath(this.plugin.settings.dashboardPath);
    if (!file) {
      root.createDiv({ cls: 'dk-empty', text: 'Dashboard not found: ' + this.plugin.settings.dashboardPath });
      return;
    }
    ...
    const visible = parsed.items.filter((it) => columns.some((c) => c.key === it.column));

    this.renderToolbar(root, visible);

    const board = root.createDiv({ cls: 'dk-board' });
    const grid = board.createDiv({ cls: 'dk-grid' });
    grid.style.gridTemplateColumns = '150px repeat(' + columns.length + ', minmax(240px, 1fr))';
    ...
    for (const col of columns) {
      ...
      if (col.key === 'tomorrow' && sub.length) h.createSpan({ cls: 'dk-colhead-sub', text: sub[0] });
      if (col.key === 'today' && sub.length) h.createSpan({ cls: 'dk-colhead-sub', text: sub[0] });
      if (col.key === 'future' && sub.length) h.createSpan({ cls: 'dk-colhead-sub', text: sub.length + ' day' + (sub.length === 1 ? '' : 's') });
    }

    const clients = this.clientRows(visible, columns);
    if (!clients.length) {
      grid.createDiv({ cls: 'dk-empty', text: 'Nothing under ## Focus in this view.' });
      return;
    }
    ...
    for (const client of clients) {
      ...
      const label = grid.createDiv({ cls: 'dk-rowhead' + (isCollapsed ? ' dk-rowhead-collapsed' : '') });
      label.createSpan({ cls: 'dk-caret', text: isCollapsed ? '\u25B8' : '\u25BE' });
      label.createSpan({ cls: 'dk-rowhead-name', text: client });
      const open = visible.filter((it) => it.client === client && !it.checked).length;
      ...
      for (const col of columns) {
        const cell = grid.createDiv({ cls: 'dk-cell dk-col-' + col.key + (isCollapsed ? ' dk-cell-collapsed' : '') });
        ...
        const cellItems = visible
          ...
          .sort((a, b) => (a.checked === b.checked ? a.order - b.order : a.checked ? 1 : -1));
        ...
        if (isCollapsed) {
          if (cellItems.length) cell.createDiv({ cls: 'dk-collapsed-count', text: String(cellItems.length) });
          continue;
        }

        if (!cellItems.length) { cell.addClass('dk-cell-empty'); continue; }
  ...
  async runAction(button, key) {
    ...
    const preview = await busy(async () => {
      ...
      for (const step of steps) {
        const r = await runScript(this.plugin, step.script, [...step.flags, '--dry-run', '--verbose']);
    ...
    if (preview.failed) {
      new Notice(label + ' failed: ' + (preview.failed.err || 'no output').split('\n')[0], 8000);
      return;
    }
    ...
    new ConfirmRunModal(this.app, label, preview.lines, async () => {
      ...
      if (result.failed) {
        new Notice(label + ' failed: ' + (result.failed.err || 'no output').split('\n')[0], 8000);
        return;
      }
  ...
  async toggleClient(client) {
    ...
    if (list.has(client)) list.delete(client); else list.add(client);
  ...
  clientRows(items, columns) {
    ...
    for (const name of configured) if (items.some((i) => i.client === name)) seen.push(name);
    for (const col of columns) {
  ...
  renderMd(el, md) {
    ...
    if (MarkdownRenderer.render) MarkdownRenderer.render(this.app, md, el, sourcePath, this);
    else MarkdownRenderer.renderMarkdown(md, el, sourcePath, this);
  }

  renderCard(cell, it) {
    const card = cell.createDiv({ cls: 'dk-card' + (it.checked ? ' dk-done' : '') });
    card.draggable = true;
    card.dataset.line = String(it.fileLine);

    card.addEventListener('dragstart', (ev) => {
      ev.dataTransfer.setData('text/plain', JSON.stringify({ line: it.fileLine, raw: it.raw, client: it.client }));
    ...
    box.addEventListener('click', (ev) => { ev.stopPropagation(); this.toggle(it); });
    ...
    if (body) {
      ...
      b.addEventListener('click', (ev) => {
        ...
        if (b.hasClass('dk-body-open')) this.expanded.add(it.fileLine); else this.expanded.delete(it.fileLine);
      });
    }
    ...
    card.addEventListener('contextmenu', (ev) => {
      ev.preventDefault();
      const menu = new Menu();
      menu.addItem((i) => i.setTitle('Reveal in Dashboard.md').setIcon('file-text').onClick(() => this.reveal(it)));
      menu.addItem((i) => i.setTitle(it.checked ? 'Mark as open' : 'Mark as done').setIcon('check').onClick(() => this.toggle(it)));
      for (const col of COLUMNS) {
        if (col.key === it.column) continue;
        menu.addItem((i) => i.setTitle('Move to ' + col.label).setIcon('arrow-right').onClick(() => this.move(it, col.key, it.client)));
      }
        ...
        if (menuHasSubmenus()) {
          menu.addItem((i) => {
            ...
            for (const name of others) {
              sub.addItem((s) => s.setTitle(name).onClick(() => this.move(it, it.column, name)));
            }
          });
        } else {
          for (const name of others) {
            menu.addItem((i) => i.setTitle('Move to ' + name).setIcon('users').onClick(() => this.move(it, it.column, name)));
          }
        }
      }
  ...
  wireDrop(cell, column, client) {
    cell.addEventListener('dragover', (ev) => { ev.preventDefault(); cell.addClass('dk-over'); });
    ...
    cell.addEventListener('drop', async (ev) => {
      ...
      const it = (this.parsed.items || []).find((x) => x.fileLine === payload.line);
      if (!it) return;
  ...
  async toggle(it) {
    ...
    await editDashboard(this.plugin, it.fileLine, it.raw, (lines) => {
      lines[it.fileLine] = lines[it.fileLine].replace(/-\s+\[[ xX]\]/, '- ' + next);
      return lines;
    });
    this.plugin.refreshViews();
  }
  ...
  async move(it, column, client) {
    const ok = await editDashboard(this.plugin, it.fileLine, it.raw, (lines) => {
      const body = it.raw.replace(/^>\s?/, '');
      lines.splice(it.fileLine, 1);
      const spot = locateInsertPoint(lines, column, client);
      if (!spot) { new Notice('No "' + column + '" day found in the dashboard.'); return null; }
  ...
  renderToolbar(root, visible) {
    ...
    const hide = bar.createEl('button', { cls: 'dk-btn', text: this.plugin.settings.hideCompleted ? 'Show done' : 'Hide done' });
...
class DashboardKanbanPlugin extends Plugin {
  async onload() {
    ...
    this.addRibbonIcon('layout-dashboard', 'Dashboard kanban', () => this.activate());
    this.addCommand({ id: 'open', name: 'Open dashboard kanban', callback: () => this.activate() });
...
class KanbanSettingTab extends PluginSettingTab {
  ...
  display() {
    ...
    new Setting(containerEl)
      ...
      .addText((t) => t.setValue(this.plugin.settings.dashboardPath).onChange(async (v) => {
    ...
    new Setting(containerEl)
      ...
      .setDesc('Comma-separated client names, in the row order you want. Clients not listed follow, in the order they appear in the file.')
      .addText((t) => t.setValue((this.plugin.settings.clientOrder || []).join(', ')).onChange(async (v) => {
        this.plugin.settings.clientOrder = v.split(',').map((s) => s.trim()).filter(Boolean);
        await this.plugin.saveSettings();
        this.plugin.refreshViews();
      }));
    ...
    new Setting(containerEl)
      ...
      .setDesc('Where the Archive and Roll scripts live. Empty uses ~/Repo/ai-toolkit.')
      .addText((t) => t.setValue(this.plugin.settings.toolkitDir).onChange(async (v) => {
    ...
    new Setting(containerEl)
      .setName('bun path')
      .setDesc('Absolute path to the bun binary. Empty searches mise, ~/.bun and Homebrew.')
      .addText((t) => t.setValue(this.plugin.settings.bunPath).onChange(async (v) => {
    ...
    new Setting(containerEl)
      ...
      .setDesc('Absolute path for a copy of the file taken before every write. Empty uses ~/.claude/dashboard-snapshots.')
      .addText((t) => t.setValue(this.plugin.settings.snapshotDir).onChange(async (v) => {
