...
list)
	...
	while :; do
		...
		printf '%s' "$PAGE" | jq -r '.data.repository.pullRequest.reviewThreads.nodes[] | select(.isResolved|not) | "\(.id)\t\(.comments.nodes[0].databaseId)\t\(.path):\(.line // .originalLine // "file")\(if .isOutdated then " (outdated)" else "" end)\t\(.comments.nodes[0].author.login)\t\(.comments.nodes[0].body | gsub("[[:space:]]+";" ") | .[0:140])"'
