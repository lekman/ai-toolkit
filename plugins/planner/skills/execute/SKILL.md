---
name: execute
description: Execute an approved goal autonomously. Read the Test Plan (Definition of Done) from the initiative's vault subpage, iterate independently until every evidence base is green through the deployed system, then open the PR and run the validation gates. Plan the goal first with /planner:goal. Use for "execute the goal", "drive this to done".
user-invocable: true
---
...
written and approved via [`/planner:goal`](../goal/SKILL.md).
...
- **The operator owns the _what_ and the _what proves it_**, already captured
  as the acceptance criteria and Test Plan by `/planner:goal`. They do not direct
  the steps.
