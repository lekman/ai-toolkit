...
sandbox inside a container requires relaxing the container's own seccomp
profile: bubblewrap needs to create a user namespace, and the default profile
blocks that. You are loosening the outer boundary to gain the inner one. That is a
