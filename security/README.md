...
- **[machine/](machine/README.md)**: workstation-level deny rules that stop the
...
- **[isolated/](isolated/README.md)**: repo-level guard rails for a disposable
  machine (remote / Docker / VM), where the agent must ask a human before
  relaxing the commit checks or reconfiguring itself. Protects the repo, not the machine. Also
